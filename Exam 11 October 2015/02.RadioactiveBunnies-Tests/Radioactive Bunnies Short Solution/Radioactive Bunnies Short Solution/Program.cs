...
namespace Radioactive_Bunnies_Short_Solution
{
    class Program
    {
        public static void Main()
        {
            ...
            foreach (char move in directions)
            {
                ...
                bunnyLair = SpreadBunnies(bunnyLair, rows, cols);

                PrintResult(bunnyLair, playerRow, playerCol, "whatever");
            ...
            for (int row = 0; row < rows; row++)
            {
                for (int col = 0; col < cols; col++)
                {
                    if (matrix[row][col] == 'B')
                    {
                        ...
                        if (row < rows && row + 1 < rows) newLair[row + 1][col] = 'B';
                        if (col > 0) newLair[row][col - 1] = 'B';
                        if (col < cols && col + 1 < cols) newLair[row][col + 1] = 'B';
                    }
                }
            }
